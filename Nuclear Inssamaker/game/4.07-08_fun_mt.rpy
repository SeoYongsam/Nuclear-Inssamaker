...
label fun_mt:
    ...
    if fun_mt_vote_day < 33 :
        ...
        Samyong "뻔대가 이렇게 늦어서 쓰냐...\n"
        extend "장발하는 애들 거의 미리 와서 기다리고 있었는데"
        Jinil "그러니까 말이야\n"
        extend "벌로 오늘 넌 가자마자 병샷이다"
        Player "으악 그것만은 참아줘..하하하하"
        Ohduck "어이어이~\n"
        extend "동아리 일정까지 미루고 온 와타시도 있다고~"
        Hyunjae "그러니까 말이야\ㅜ"
        extend "그래도 뻔대 왔으니까 다 도착한 거 맞지?"
        Mirae "그런거 같아~\n"
        extend "그럼 슬슬 출발할까??"
        Player "오키오키~ 출발하자!!"
    ...
    #2. 장발 투표를 4/4 이후로 끝냈을 시(4월 4일, 4월 5일에 종료 하였을 경우)
        ...
        extend "이 쯤이면 다 모인 것 같으니까 슬슬 출발할까?"
        Jinil "뭐 그래 어쩔 수 없지\n"
        extend "이렇게 된 거 안주 같은거 우리가 먹고 싶은걸로 다 사자!"
        Samyong "나는 통큰팝콘~"
    ...
    "예산에 맞게 장을 보던 중 낙성대학교 학생들이 과잠을 입고 장을 보고 있는 모습을 보게되었다."
    ...
    if gwazam_hidden == True :
        ...
        Jangjung "하 그래도 어제 안 왔으면 못입고 왔을 뻔.."
    ...
    #4. (과잠 히든을 달성하지 못한 경우) (21일 이후에 과잠 투표를 마감할 경우 아래의 스크립트)
        ...
        Samyong "에이 뻔대가 미안할 건 아니지"
        Jinil "맞음 애들이 투표에 참가 잘 안 한게 잘못이지"
    ...
    if gwa_parameter >= 20 :
        ...
        Player "아녀아녀 괜찮아\n"
        extend "맛있다니 다행이네"
        Hyunjae "아 저기 고기 다 떨어진 것 같은데, 꺼내 줘야겠다\n"
        extend  "(부스럭부스럭) 여기!"
        Player "오 땡큐~ 일단 앉아있어\n"
        extend "고기 굽는데 조금 더 걸리니까"
    ...
    Player "오키오키! 자 문정과 19학번, 아직 밤은 기니까 신나게 마셔보자!\n"
    extend "자 건배!!"
    "친구들" "건배!!!!!!"
    ...
    Jinil "어! 우리 뻔대~~"
    Player "휴.. 찾았다... 야!\n"
    extend "진일아 어쩌다 여기까지 왔냐;; 아니다 일단 돌아가자."
...
label fun_morning:
    ...
    if rest_point == 3 :
        ...
        "밖을 보니, 첫 차를 타고 간 아이들이 몇 명 정도 있는 것 같다."
        "남은 아이들은 정리를 거의 끝내둔 것 같다."
        ...
        show small_room at truecenter
        Player "으으으..머리야...시간이..?\n"
        extend "음.. 체크아웃 시간까지는 조금 남았지만 슬슬 애들 깨울까..?"
        Player "얘들아! 일어나!! 슬슬 집에가자!!"
        Player "다들 짐 챙기고~ 잃어버린 거 없이 다 챙겨서 나가자!"
        ...
        show big_room at truecenter
        "방은 그래도 먼저 간 친구들이 치워준 모양인지 생각보다 깔끔했다..."
